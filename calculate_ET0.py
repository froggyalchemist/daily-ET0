...
def kelvin_to_celsius(da: xr.DataArray) -> xr.DataArray:
    ...
    if da.attrs.get("units") == "K":
        ...
        raise AttributeError(
            f"Temperature must be in Kelvin but units are {da.attrs.get("units")}"
        )


def net_surface_radiation(
    rsds: xr.DataArray, rsus: xr.DataArray, rlds: xr.DataArray, rlus: xr.DataArray
) -> xr.DataArray:
    """Rn = (rsds - rsus) + (rlds - rlus) (W/m²), converted to MJ/m²/day."""
    if (
        rsds.attrs.get("units") == "W m-2"
        and rsus.attrs.get("units") == "W m-2"
        and rlds.attrs.get("units") == "W m-2"
        and rlus.attrs.get("units") == "W m-2"
    ):
        return ((rsds - rsus) + (rlds - rlus)) * 86400 / 1e6
    else:
        raise ValueError(
            f"Inputs must be in W m-2 but units are {rsds.attrs.get("units")}, {rsus.attrs.get("units")}"
        )
...
def saturation_vapor_pressure(tas: xr.DataArray) -> xr.DataArray:
    ...
    if tas.attrs.get("units") != "C":
        raise ValueError(
            f"Temperature must be in ºC but it is in {tas.attrs.get("units")}"
        )
...
def wind_speed_2m(sfcWind: xr.DataArray, height: float = 10.0) -> xr.DataArray:
    ...
    if sfcWind.attrs.get("units") != "m s-1":
        raise ValueError(
            f"Wind speed must be in m s-1 but it is in {sfcWind.attrs.get("units")}"
        )
...
def penman_monteith(
    ...
    ET0rad.attrs = {"units": "mm day-1", "long_name": "Radiative component of ET0"}
    ET0adv.attrs = {"units": "mm day-1", "long_name": "Advective component of ET0"}
    VPD.attrs = {"units": "kPa", "long_name": "Vapor pressure deficit"}
...
def process_combination(
    archive, gcm, exp, chunks, output_dir: str = DEFAULT_OUTPUT_DIR
):
    ...
    if exp not in ca.EXPERIMENTS:
        raise ValueError(
            f"Experiment '{exp}' is not in EXPERIMENTS. Modify cmip6_archive.py if necessary."
        )

    # Read in data as Xarray DataArrays
    tas_ds = archive.get_variable_dataset(
        gcm, exp, "tas", chunks=chunks
    )  # Used to copy dataset-level attributes from parent GCM
    tas = tas_ds["tas"]
    ...
    sfcWind = archive.get_variable_dataset(gcm, exp, "sfcWind", chunks=chunks)[
        "sfcWind"
    ]
    ...
    ds = penman_monteith(
        tas, ps, rsds, rsus, rlds, rlus, sfcWind, hurs, parent_ds_attrs=tas_ds.attrs
    ).persist()
